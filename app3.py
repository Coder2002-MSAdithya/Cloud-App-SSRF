...
def index():
    ...
    <html>
      ...
      <body>
        <h1>Image Resizer / SSRF Demo Service [Service 3]</h1>
        <p>
...
def resize():
    ...
    if not url:
        return "[Service 3] No URL\n", 400

    # SSRF protection (IP-based)
    if not ssrf_protect.is_safe(url):
        return "[Service 3] BLOCKED by SSRFProtection\n", 403
    
    try:
        print(f"[Service 3] FETCHING → {url}")
    ...
    if is_html_content(resp):
        ...
        if title:
            return f"[Service 3] HTML title: {title}", 200
    ...
    if is_image_content(resp):
        try:
            ...
            return f"[Service 3] Failed to process image: {e}\n", 500

    # Unknown type
    return f"[Service 3] Unknown or unsupported Content-Type: {resp.headers.get('Content-Type')}\n", 400
