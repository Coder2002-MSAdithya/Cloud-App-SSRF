...
def index():
    ...
    <html>
      ...
      <body>
        <h1>Image Resizer / SSRF Demo Service [Service 1]</h1>
        <p>
...
def resize():
    ...
    try:
        ...
        return f"[Service 1] Failed to fetch: {e}\n", 502
    ...
    if is_html_content(resp):
        ...
        if title:
            return f"[Service 1] HTML title: {title}", 200
        return "[Service 1] HTML document has no detectable <title>\n", 200
    ...
    if is_image_content(resp):
        try:
            ...
            return f"[Service 1] Failed to process image: {e}\n", 500

    # Unknown type
    return f"[Service 1] Unknown or unsupported Content-Type: {resp.headers.get('Content-Type')}\n", 400


if __name__ == "__main__":
    app.run(host="10.10.0.10", port=80, debug=False)